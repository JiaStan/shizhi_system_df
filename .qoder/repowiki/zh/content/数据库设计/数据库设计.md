...
现在我已经收集了足够的信息来更新数据库设计文档。基于代码分析，我发现了以下重大变更：

1. **新增了四个核心表**：
   - `gantt_schedules`（甘特排程计划表）
   - `gantt_resource_allocations`（甘特资源分配表） 
   - `gantt_conflicts`（甘特资源冲突表）
   - 增强了 `alerts` 表的SLA管理功能

2. **增强了现有表的功能**：
   - alerts表增加了多种预警类型和SLA管理机制
   - 所有新表都包含完整的索引设计和业务逻辑
...
## 更新摘要
**所做更改**
- 新增甘特排程相关三个核心表：gantt_schedules、gantt_resource_allocations、gantt_conflicts
- 增强alerts表的SLA管理和多类型预警支持
- 新增完整的甘特图排程API接口和服务层实现
- 更新ER图和表关系以反映新的数据模型
- 新增冲突检测和关键路径计算的数据库支持
...
## 引言
本文件面向"试制资源数智化管理系统"的数据库设计与使用，覆盖设备台账、车间区域、人员、任务、异常预警、人员效率统计、设备维护记录以及**新增的甘特排程、资源分配和冲突检测**等核心表的结构设计、字段含义、约束与索引、表间关系、数据初始化流程、版本管理策略、访问模式与性能优化建议，并提供常用SQL查询示例与最佳实践。文档同时给出ER图与表关系图，帮助读者快速理解整体数据模型与业务流转。

## 项目结构
后端通过Python脚本执行SQL建表与初始化数据，数据库连接由统一模块提供，服务层按领域拆分（设备、人员、任务、预警、甘特排程），并基于Pydantic进行请求/响应校验。
...
G["服务层<br/>equipment/personnel/task/alert/gantt services"] --> E
H["路由层<br/>router.py"] --> G
```
...
- [backend/modules/resource/router.py:1-1565](file://backend/modules/resource/router.py#L1-L1565)
...
- 异常预警 alerts：**增强版**预警编号、类型（9种预警类型）、级别、标题、描述、来源、关联对象、发生区域/场地、**SLA管理**、升级、处理状态、处置方案、损失与影响工时等。
- 人员效率统计 personnel_efficiency：按日统计人员总工时、有效工时、完成任务数、设备使用率、主要工作区域等。
- 设备维护记录 equipment_maintenance：设备编号、维护类型、起止时间、维护人、描述、状态。
- **甘特排程 gantt_schedules**：排程编号、关联任务、阶段、优先级、状态、场地/区域、占用设备/举升机数量、计划/实际起止时间、工时、进度、层级、**约束处理**、关键路径、冲突标记等。
- **甘特资源分配 gantt_resource_allocations**：排程ID、资源类型/编码/名称、占用起止时间、分配工时、数量、优先级、状态等。
- **甘特资源冲突 gantt_conflicts**：冲突编号、类型（6种冲突类型）、严重级别、关联排程A/B、冲突资源、重叠时段、建议、处理状态、解决信息等。

**更新** 新增了完整的甘特排程管理系统，包括排程计划、资源分配和冲突检测三大核心功能。

章节来源
- [backend/sql/resource_schema.sql:11-326](file://backend/sql/resource_schema.sql#L11-L326)

## 架构总览
系统采用"脚本化DDL + 初始化数据 + 服务层封装 + 连接池"的分层架构。数据库为MariaDB/MySQL兼容，字符集utf8mb4；连接池默认最大连接10，最小缓存0，阻塞获取；所有写操作均带事务提交与回滚保护。**新增的甘特排程功能提供了完整的CRUD操作、冲突检测算法和关键路径计算能力**。

```mermaid
sequenceDiagram
participant Admin as "管理员/调度员"
participant API as "API路由"
participant Svc as "服务层(设备/人员/任务/预警/甘特)"
participant DBP as "数据库连接池(database.py)"
participant DB as "数据库(MariaDB)"
Admin->>API : 调用CRUD或统计接口
API->>Svc : 调用服务方法
Svc->>DBP : get_conn()
DBP-->>Svc : 返回连接
...
- [backend/modules/resource/services/gantt_service.py:1-200](file://backend/modules/resource/services/gantt_service.py#L1-L200)
- [backend/modules/resource/router.py:1195-1420](file://backend/modules/resource/router.py#L1195-L1420)
...
- **alerts（增强版）**
  - 主键：id（自增）
  - 唯一键：alert_code
  - **新增字段**：alert_type（9种预警类型：task_delay、quality_defect、equipment_fault、material_shortage、personnel_gap、safety_hazard、schedule_overdue、process_violation、external_coordinate、other）、sla_hours（SLA响应时长）、escalated（升级标记）、escalated_to（升级对象）
  - **SLA管理**：根据level自动设置默认SLA（critical=4h, high=8h, medium=24h, low=72h）
...
- **gantt_schedules（新增）**
  - 主键：id（自增）
  - 唯一键：schedule_code
  - **关键字段**：task_id/code（关联任务）、phase（装配/下线/调试/夏标验证/整改/终检/交付）、constraint_type（约束类型：none、start_no_earlier、end_no_later、must_on、as_soon_as_possible）、predecessor_ids（前置依赖）、is_critical（关键路径标记）、slack_hours（浮动时差）、has_conflict/conflict_count（冲突标记）
  - **索引**：task组合、type、status、priority、site、is_critical、has_conflict、plan_start/end、parent_id
- **gantt_resource_allocations（新增）**
  - 主键：id（自增）
  - 唯一键：allocation_code
  - **关键字段**：schedule_id/code（关联排程）、resource_type（equipment/personnel/zone/lift/material/other）、resource_code/name、start/end_time、hours_allocated、quantity、priority、status
  - **索引**：schedule组合、resource组合、status、start/end
- **gantt_conflicts（新增）**
  - 主键：id（自增）
  - 唯一键：conflict_code
  - **关键字段**：conflict_type（6种冲突类型：resource_overlap、time_overlap、dependency_miss、deadline_risk、overload、other）、severity（严重级别）、schedule_a/b（关联排程）、resource_type/code/name（冲突资源）、overlap_start/end/hours（重叠时段）、suggestion（处理建议）、status/resolution（处理状态）
  - **索引**：conflict_type、severity、status、schedule_a/b、resource组合、assembly_site、detected_at

**更新** 新增了完整的甘特排程管理系统，包括排程计划、资源分配和冲突检测功能，支持复杂的项目排程和资源优化。
...
- 执行顺序
  ...
  2) 执行 resource_schema.sql 创建所有表与索引（**包含新增的甘特排程相关表**）
...
ExecSchema -- 是 --> RunSchema["执行建表脚本<br/>包含甘特排程表"]
...
- 服务层
  ...
  - **alert_service（增强版）**：预警分页查询、详情、创建/更新/升级、批量状态更新、**SLA超期计算**、统计
  - **gantt_service（新增）**：甘特排程CRUD、资源分配管理、冲突检测、关键路径计算、统计报表
...
participant GS as "GanttService"
participant DB as "Database Pool"
API->>ES : 设备列表/详情/维护
API->>PS : 人员列表/详情/看板
API->>TS : 任务列表/详情/统计
API->>AS : 预警列表/详情/升级
API->>GS : 甘特排程/资源分配/冲突检测
ES->>DB : 执行SQL
PS->>DB : 执行SQL
TS->>DB : 执行SQL
AS->>DB : 执行SQL
...
- 服务层对表的耦合
  ...
  - **alert_service 强依赖 alerts、zones（新增SLA管理逻辑）**
  - **gantt_service 强依赖 gantt_schedules、gantt_resource_allocations、gantt_conflicts、tasks（新增排程管理逻辑）**
...
- 建议
  ...
  - 定期分析慢查询日志，调整索引与SQL
  - **监控甘特排程冲突检测的性能，必要时添加缓存机制**

**更新** 新增了甘特排程相关的性能优化建议，包括冲突检测的缓存机制和时间范围查询优化。
...
- **甘特排程相关问题**
  - **冲突检测失败：检查gantt_resource_allocations表的数据完整性**
  - **关键路径计算异常：验证predecessor_ids的依赖关系是否存在环**
  - **资源分配冲突：检查同一时间段内相同资源的分配情况**

**更新** 新增了甘特排程相关的故障排查指南，包括冲突检测、关键路径计算和资源分配问题的诊断方法。
...
## 结论
本数据库设计围绕试制资源管理的核心实体展开，涵盖设备、区域、人员、任务、预警、效率统计、维护记录以及**新增的甘特排程、资源分配和冲突检测**。通过合理的索引与连接池配置，满足日常读写与统计需求。**新增的甘特排程功能提供了完整的项目排程管理能力，包括约束处理、关键路径计算和资源冲突检测**。建议在生产环境中引入版本化迁移工具，完善审计与监控，持续优化慢查询与索引策略。

**更新** 系统现已具备完整的项目排程和资源管理能力，能够支持复杂的试制生产场景。
...
- **甘特排程数据查询（支持筛选、分页、日期区间）**
  - 参考路径：[gantt_service.py:176-200](file://backend/modules/resource/services/gantt_service.py#L176-L200)
- **资源分配列表查询**
  - 参考路径：[gantt_service.py:1302-1316](file://backend/modules/resource/services/gantt_service.py#L1302-L1316)
- **冲突检测与处理**
  - 参考路径：[gantt_service.py:786-830](file://backend/modules/resource/services/gantt_service.py#L786-L830)
- 创建设备维护记录
...
**更新** 新增了甘特排程相关的SQL查询示例，包括排程数据查询、资源分配和冲突检测。
...
- **甘特排程相关最佳实践**：
  - **合理设置约束类型，避免过于严格的约束导致排程困难**
  - **定期执行冲突检测，及时识别和解决资源冲突**
  - **利用关键路径分析优化项目进度**
  - **批量操作资源分配，提高排程效率**

**更新** 新增了甘特排程相关的最佳实践建议，帮助开发者更好地使用新增的排程功能。

### API接口概览
- **设备管理**：GET/POST/PUT/DELETE /api/resource/equipment
- **人员管理**：GET/POST/PUT/DELETE /api/resource/personnel  
- **任务管理**：GET/POST/PUT/DELETE /api/resource/tasks
- **预警管理**：GET/POST/PUT /api/resource/alerts
- **甘特排程管理**：
  - GET/POST/PUT/DELETE /api/resource/gantt/schedules
  - GET/POST /api/resource/gantt/allocations
  - POST /api/resource/gantt/check-conflicts
  - GET/PUT /api/resource/gantt/conflicts
  - POST /api/resource/gantt/compute-critical

**更新** 新增了完整的甘特排程API接口，支持项目排程的全生命周期管理。
