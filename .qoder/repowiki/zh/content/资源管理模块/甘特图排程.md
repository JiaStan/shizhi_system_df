...
## 更新摘要
**所做更改**
- 更新了冲突检测算法实现，支持多类型资源重叠检测
- 增强了资源分配优化功能，支持批量分配和智能推荐
- 实现了多项目并行调度，支持不同装配场地的独立调度
- 完善了BFWS（带时间窗的分支定界）算法的工程化实现
- 添加了关键路径计算和松弛时间分析功能
...
## 简介
本模块提供试制资源数智化管理系统中的"甘特图排程"能力，涵盖排程数据管理、资源冲突检测、关键路径计算、状态批量更新等后端服务，以及前端可视化与交互。系统实现了BFWS（带时间窗的分支定界）+ CP-SAT的智能排程求解方案，用于在复杂约束下生成全局可行的装配/调试/交付计划，并通过甘特图进行可视化呈现与人工微调。

**最新更新**：增强了复杂冲突检测算法，支持资源重叠、依赖错过、截止风险、同场地设备/区域重叠等多种冲突类型；实现了资源分配优化，支持批量分配和智能建议；支持多项目并行调度，可在不同装配场地同时进行独立排程。
...
SVC --> BFWS["BFWS算法引擎<br/>冲突检测 + 关键路径"]
SVC --> OPT["资源优化引擎<br/>分配优化 + 多项目调度"]
```
...
## 核心组件
- **甘特服务层**：实现排程 CRUD、资源分配、冲突检测、关键路径计算、批量状态更新等核心功能。
...
- **BFWS算法引擎**：实现带时间窗的分支定界算法，支持复杂约束下的全局优化。
- **资源优化引擎**：提供资源分配优化和多项目并行调度能力。
...
- **资源分配优化**
  ...
  - 智能分配建议：基于历史数据和约束条件推荐最优分配方案。

- **复杂冲突检测（detect_conflicts）**
  - **资源重叠检测**：按资源类型+编码分组，两两比较时间段是否重叠，计算重叠小时数并判定严重级别。
  - **依赖错过检测**：检查前置任务的结束时间是否晚于当前任务开始时间。
  - **截止风险评估**：对未完成且超期的任务标记风险，支持SLA监控。
  - **同场地设备/区域重叠**：同一装配场地的设备或区域在同一时段被多个任务占用。
  - **多项目并行冲突**：跨项目的资源竞争检测和协调。
  - 可选自动保存：将冲突写入 gantt_conflicts，并回写排程的 has_conflict/conflict_count。

- **关键路径计算（compute_critical_path）**
  - 基于拓扑排序（Kahn算法）构建 ES/EF/LF/LS，计算松弛时间，标记 is_critical 与 slack_hours。
  - 支持多项目并行环境下的关键路径识别。
  - 实时更新：排程变更时自动重新计算关键路径。
...
Severity --> CheckDep{"检查依赖关系"}
CheckDep --> CheckDeadline{"检查截止风险"}
CheckDeadline --> CheckSite{"检查同场地冲突"}
CheckSite --> AutoSave{"是否自动保存?"}
AutoSave -- 是 --> WriteDB["写入冲突表并更新排程计数"]
AutoSave -- 否 --> End1["返回内存冲突列表"]
WriteDB --> End1
Next1 --> CheckDep
```
...
- **索引优化**：针对冲突类型、严重级别、状态、关联排程、资源、装配场地、检测时间建立索引，提升查询与筛选性能。
- **新增字段**：支持多项目标识、SLA监控、冲突解决追踪等功能。
...
- **新增依赖**：BFWS算法引擎和资源优化引擎作为独立模块，支持插件化扩展。
...
## 结论
本模块已实现甘特图排程的核心功能：排程数据管理、资源分配、冲突检测、关键路径计算与批量状态更新，并提供完善的 REST API 与前端交互原型。**最新更新**增强了复杂冲突检测算法，支持多种冲突类型的自动识别和处理；实现了资源分配优化，提供智能建议和批量操作；支持多项目并行调度，可在不同装配场地同时进行独立排程。BFWS + CP-SAT 的设计为未来引入精确求解提供了清晰路径，当前规则引擎可作为快速模式保障生产可用性。建议在生产环境中结合场景限制检测范围、启用异步计算与缓存，以提升性能与稳定性。
