...
export default function DeliveryOverviewSection() {
  ...
  const stats = useMemo(() => {
    ...
    const pendingInspection = stateCounts['待检'] || 0;
    const unqualifiedPending = stateCounts['不合格待判定'] || 0;

    return { total, stateCounts, sourceCounts, totalIn, totalUnqualified, totalOrder, totalReceived, pendingInspection, unqualifiedPending };
  }, [filtered]);

  /* ─── KPI 卡片数据 ─── */
  const kpiCards = useMemo<IKpiCard[]>(() => {
    const deliveryRate = stats.totalOrder > 0 ? (stats.totalIn / stats.totalOrder) * 100 : 0;
    ...
    return [
      ...
      },
      {
        ...
        topColor: deliveryColor.top,
        iconBg: deliveryColor.bg,
        iconColor: deliveryColor.text,
      },
      {
        label: '待检',
        ...
        unit: '条',
        icon: ClockIcon,
        topColor: stats.pendingInspection > 0 ? 'border-t-[hsl(40_70%_50%)]' : 'border-t-[hsl(152_55%_40%)]',
        iconBg: stats.pendingInspection > 0 ? 'bg-[hsl(40_75%_94%)]' : 'bg-[hsl(152_45%_95%)]',
        iconColor: stats.pendingInspection > 0 ? 'text-[hsl(40_65%_25%)]' : 'text-[hsl(152_60%_22%)]',
      },
      {
        label: '不合格待判定',
        value: stats.unqualifiedPending,
        unit: '条',
        icon: AlertTriangleIcon,
        topColor: stats.unqualifiedPending > 0 ? 'border-t-[hsl(4_65%_48%)]' : 'border-t-[hsl(152_55%_40%)]',
        iconBg: stats.unqualifiedPending > 0 ? 'bg-[hsl(4_55%_95%)]' : 'bg-[hsl(152_45%_95%)]',
        iconColor: stats.unqualifiedPending > 0 ? 'text-[hsl(4_60%_28%)]' : 'text-[hsl(152_60%_22%)]',
      },
    ];
  }, [stats]);
  ...
  const statePieData = useMemo(() => {
    const entries = Object.entries(stats.stateCounts).filter(([, v]) => v > 0);
    return entries.map(([name, value]) => ({
      name,
      value,
      itemStyle: { color: STATE_COLORS[name] || '#a8b0bd' },
    }));
  }, [stats.stateCounts]);

  const statePieOption: EChartsOption = useMemo(() => ({
    tooltip: {
      trigger: 'item',
    ...
    legend: {
      type: 'scroll',
      bottom: 0,
      textStyle: { fontSize: 11, color: '#6b7685' },
      itemWidth: 10,
      itemHeight: 10,
      itemGap: 12,
    },
  ...
  const sourcePieData = useMemo(() => {
    const labels: Record<string, string> = { warehouse: '仓库系统', feishu: '飞书共享表' };
    return Object.entries(stats.sourceCounts)
      .filter(([, v]) => v > 0)
      .map(([key, value]) => ({
        name: labels[key] || key,
        value,
        itemStyle: { color: SOURCE_COLORS[key] || '#a8b0bd' },
      }));
  }, [stats.sourceCounts]);

  const sourcePieOption: EChartsOption = useMemo(() => ({
    tooltip: {
      trigger: 'item',
      formatter: (params: TopLevelFormatterParams) => {
        const p = params as { name: string; value: number; percent: number };
  ...
  return (
    <section className="w-full space-y-5">
      ...
      >
        ...
        <motion.div
          variants={cardVariants}
          className="bg-card border border-border rounded-sm p-4 lg:col-span-3"
        >
        ...
        <motion.div
          variants={cardVariants}
          className="bg-card border border-border rounded-sm p-4 lg:col-span-2"
        >
          <div className="mb-2">
              ...
              {(stats.sourceCounts['warehouse'] || 0) > 0 && (
                <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  <span className="font-mono font-medium text-foreground">
                    {stats.sourceCounts['warehouse'] || 0}
                  </span>
                </span>
              )}
